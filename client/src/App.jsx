...
export default function App() {
  ...
  const brandColors = { bg: '#F8F5F0', textPrimary: '#5D4037', textSecondary: '#8D6E63', accent: '#A1887F', accentLight: '#EFEBE9', footerBg: '#5D4037', footerText: '#F8F5F0', footerTextSecondary: '#BCAAA4' };
  ...
  return (
    <div className="font-sans" style={{ backgroundColor: brandColors.bg, color: brandColors.textPrimary }}>
      <header className="shadow-sm sticky top-0 z-50" style={{ backgroundColor: brandColors.bg }}>
        <nav className="container mx-auto px-6 py-2 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <a href="#" className="hover:opacity-75" style={{ color: brandColors.textSecondary }}>로그인</a>
          </div>
          <a href="#" className="hidden md:block text-white px-6 py-2 rounded-full hover:opacity-90 transition" style={{ backgroundColor: brandColors.accent }}>실시간 예약</a>
          <div className="md:hidden"><button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="focus:outline-none" style={{ color: brandColors.textPrimary }}><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg></button></div>
        </nav>
        {mobileMenuOpen && ( <div className="md:hidden py-4" style={{ backgroundColor: brandColors.bg }}><a href="#services" className="block px-6 py-2" style={{ color: brandColors.textSecondary }}>서비스</a><a href="#booking" className="block px-6 py-2" style={{ color: brandColors.textSecondary }}>예약 방법</a><a href="#about" className="block px-6 py-2" style={{ color: brandColors.textSecondary }}>소개</a><a href="#" className="block px-6 py-2" style={{ color: brandColors.textSecondary }}>로그인</a><div className="px-6 mt-2"><a href="#" className="block w-full text-center text-white px-6 py-2 rounded-full transition" style={{ backgroundColor: brandColors.accent }}>실시간 예약</a></div></div> )}
      </header>
      <main>
        ...
        <section id="services" className="py-20" style={{ backgroundColor: 'white' }}><div className="container mx-auto px-6 text-center"><h2 className="text-3xl font-bold mb-4" style={{ color: brandColors.textPrimary }}>PoshPet의 특별한 서비스</h2><p className="max-w-2xl mx-auto mb-12" style={{ color: brandColors.textSecondary }}>각 분야 최고의 전문가들이 최상의 케어를 약속합니다.</p><div className="grid md:grid-cols-3 lg:grid-cols-5 gap-8">{services.map((service, index) => ( <div key={index} className="p-8 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-2 transition-all duration-300" style={{ backgroundColor: brandColors.bg }}><div className="flex items-center justify-center h-16 w-16 rounded-full mx-auto mb-6" style={{ backgroundColor: brandColors.accentLight, color: brandColors.accent }}>{service.icon}</div><h3 className="text-xl font-semibold mb-3" style={{ color: brandColors.textPrimary }}>{service.name}</h3><p style={{ color: brandColors.textSecondary }}>{service.description}</p></div> ))}</div></div></section>
      ...
      <footer id="about" className="py-12" style={{ backgroundColor: brandColors.footerBg, color: brandColors.footerText }}><div className="container mx-auto px-6 text-center"><div className="flex justify-center mb-6"><img src="/assets/image_9791c6.jpg" alt="PoshPet Logo" className="h-16 w-auto filter invert"/></div><p className="mb-4" style={{ color: brandColors.footerTextSecondary }}>주소: 베트남 하노이시 | 대표: OOO | 사업자등록번호: 123-45-67890<br />고객센터: 02-1234-5678 | 이메일: contact@poshpet.com</p><p className="text-sm" style={{ color: '#BCAAA4' }}>&copy; 2025 PoshPet. All Rights Reserved.</p></div></footer>
    </div>
  );
}
