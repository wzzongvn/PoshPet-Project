...
export default function ReservationPage({ setCurrentPage, user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const reservationData = { petId: selectedPet, serviceId: selectedService, reservationDate: selectedSlot, notes };
        ...
        <div className="mt-10 bg-white p-8 rounded-xl shadow-lg">
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <select id="service" value={selectedService} onChange={e => setSelectedService(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-brown-500 focus:border-brown-500 sm:text-sm rounded-md">{services.map(s => <option key={s._id} value={s._id}>{s.name} - {s.price.toLocaleString()} VND</option>)}</select>
            </div>
            <div>
