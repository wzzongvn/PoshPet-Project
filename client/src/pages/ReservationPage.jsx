...
import { getMyPets, createReservation } from '../services/api.js';
...
export default function ReservationPage({ setCurrentPage, user }) {
      ...
      const fetchPets = async () => {
        try {
          ...
          if (response.data.length > 0) {
            setSelectedPet(response.data[0]._id);
          }
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const reservationData = { petId: selectedPet, serviceName: selectedService, reservationDate, notes };
  ...
  if (!user) {
    return (
      <div className="text-center py-20"><p>예약을 하시려면 먼저 로그인이 필요합니다.</p><button onClick={() => setCurrentPage('login')} className="mt-4 bg-brown-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-brown-700">로그인 페이지로 이동</button></div>
    );
  }
  ...
  return (
    <div className="bg-cream-50 py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center"><h2 className="text-3xl font-extrabold text-brown-800 font-serif">실시간 예약</h2><p className="mt-4 text-lg text-gray-500">원하시는 서비스와 시간을 선택해주세요.</p></div>
        <div className="mt-10 bg-white p-8 rounded-xl shadow-lg">
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <select id="service" value={selectedService} onChange={e => setSelectedService(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-brown-500 focus:border-brown-500 sm:text-sm rounded-md">{availableServices.map(s => <option key={s.name}>{s.name}</option>)}</select>
            </div>
            <div>
            ...
            <div className="pt-5"><div className="flex justify-end"><button type="button" onClick={() => setCurrentPage('home')} className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brown-500">취소</button><button type="submit" className="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brown-600 hover:bg-brown-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brown-500">예약 신청하기</button></div></div>
          </form>
        </div>
      </div>
    </div>
