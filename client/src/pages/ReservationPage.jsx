...
export default function ReservationPage({ setCurrentPage, user }) {
  ...
  // 1. 페이지 로드 시, 펫 목록과 서비스 목록을 불러옵니다.
  useEffect(() => {
    if (user) {
  ...
  // 2. 날짜(selectedDate)가 변경될 때마다, 해당 날짜의 예약 가능 시간을 불러옵니다.
  useEffect(() => {
    if (selectedDate) {
      const fetchSlots = async () => {
        try {
          ...
          setSelectedSlot(''); // 날짜가 바뀌면 선택된 시간 초기화
        } catch (err) {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 bg-white p-8 rounded-xl shadow-lg">
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div className="pt-5"><div className="flex justify-end">
