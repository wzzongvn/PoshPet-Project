/*
* =======================================================================
* 파일: client/src/pages/MyPageComponent.jsx (수정)
* =======================================================================
* 설명: 이제 이 페이지는 반려동물 정보뿐만 아니라, 예약 내역도 함께 보여줍니다.
* 페이지가 로드될 때 서버로부터 예약 내역을 가져와 목록으로 표시합니다.
*/
import React, { useState, useEffect } from 'react';
// ★★★ 예약 내역을 가져오기 위한 함수를 import 합니다. ★★★
import { getMyPets, addPet, getMyReservations } from '../services/api.js';

export default function MyPageComponent({ user }) {
  const [pets, setPets] = useState([]);
  // ★★★ 예약 내역을 저장할 새로운 상태(state)를 추가합니다. ★★★
  ...
  // 페이지가 로드될 때 내 반려동물 목록과 예약 내역을 모두 불러옵니다.
  useEffect(() => {
            ...
            {reservations.length > 0 ? (
              reservations.map(res => (
                <div key={res._id} className="border border-gray-200 rounded-lg p-4 flex justify-between items-center">
                  <div>
                    <p className="font-bold text-brown-800">{res.serviceName}</p>
